...
test.describe('Multi-Player Gameplay', () => {
  test('should synchronize moves between two players', async ({ page, context }) => {
    ...
    await expect(blackPage.locator(selectors.game.currentTurn)).toBeVisible();
  ...
  test('should show real-time turn updates', async ({ page, context }) => {
    ...
    expect(whiteTurnIndicator).toContain('Your turn');
    expect(blackTurnIndicator).toContain("Opponent's turn");
